...
switch (functionChoice) {
  ...
  case 3: // truncateString
    if (args.length < 3) {
      console.error(
        "Per la funzione 'Tronca Stringa' (3), è necessaria una lunghezza massima."
      );
      console.error(
        "Utilizzo: node index.js 3 <stringa_input> <lunghezza_massima>"
      );
      process.exit(1);
    }
    ...
    if (isNaN(maxLength) || maxLength < 0) {
      console.error(
        "La lunghezza massima per troncare deve essere un numero positivo."
      );
      process.exit(1);
    }
  ...
  default:
    console.error(
      `Numero funzione non valido: ${functionChoice}. Scegliere un numero da 1 a 4.`
    );
    process.exit(1);
}
